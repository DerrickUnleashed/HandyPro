...
def customer_service_history():
    ...
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute("SELECT RequestID,Name,Email,ServiceName,RequestDate,Status,Rating,Review FROM ServiceRequests sr JOIN Services s ON sr.ServiceID = s.ServiceID JOIN Professionals p ON p.ProfessionalID = sr.ProfessionalID WHERE CustomerID = ?", (customer_id,))
    ...
    if request.method == 'POST':
        ...
        print(request_id,rating,review)
        if request_id and rating and review:
            conn = sqlite3.connect('database.db')
            cursor = conn.cursor()
            try:
                cursor.execute("UPDATE ServiceRequests SET Rating = ?, Review = ?, Status = ? WHERE RequestID = ?", (rating, review,"Closed", request_id))
                conn.commit()
